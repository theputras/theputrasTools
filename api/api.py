...
from controller.PaymentController import (
    ...
    handle_check_status,
    handle_create_qris,
    handle_generate_qr_merchant,
    handle_list_transactions,
)
from controller.PrayerController import (
    fetch_global_hijri_calendar,
    get_islamic_calendar_for_user,
    get_prayer_schedule_for_user,
    get_ramadan_calendar_for_user,
    reverse_geocode,
    search_location,
)
from models.googleOuth import (
    google_cal_service,  # Import inside function to avoid circular import if necessary
)
...
def init_api(
    ...
    logfile,
    valid_role_func,
    sskm_rooms_storage=None,
):
    global \
        photo_cache, \
        majorID, \
        executor, \
        get_jadwal_status_func, \
        log_file, \
        _valid_role, \
        SSKM_ROOMS
    photo_cache = cache
...
def my_hook(d, task_id):
    if task_id in download_progress and download_progress[task_id].get(
        "cancelled", False
    ):
    ...
    if d["status"] == "downloading":
        ...
        try:
            ...
            progress_val = 0.0
        logging.debug(
            f"[DEBUG HOOK] Raw: {raw_p} -> Clean: {progress_val}% | Speed: {clean_s}"
        )
...
def api_status():
    ...
    if not user_id:
        ...
        return jsonify({"status": "error", "message": "Anda belum login."})
...
def api_search():
    ...
    if not df_mahasiswa.empty:
        for _, row in df_mahasiswa.iterrows():
            ...
            if majorID:
                prodi_name = (
                    majorID.get(nim[2:7], "Prodi Tidak Dikenal")
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            combined_qualities = sorted(
                list(
                    set([f.get("height") for f in combined_formats if f.get("height")])
                ),
                reverse=True,
            )
            # Gabungin semua kualitas video
            all_video_qualities = sorted(
                list(set(video_qualities + [f"{r}p" for r in combined_qualities])),
                reverse=True,
            )
            ...
            if audio_bitrates:
                audio_qualities.append(
                    {"id": "best", "label": f"Best (≈{int(audio_bitrates[0])}k)"}
                )
                if len(audio_bitrates) > 1:
                    audio_qualities.append(
                        {
                            "id": "medium",
                            "label": f"Medium (≈{int(audio_bitrates[-1])}k)",
                        }
                    )
            else:
        ...
        return jsonify(
            {
                "error": f"Gagal mengambil info video. Mungkin video ini private atau dihapus."
            }
        ), 500
    except Exception as e:
        logging.error(f"[yt-dlp info] Error: {e}")
        return jsonify(
            {"error": "Terjadi kesalahan internal saat mengambil info video."}
        ), 500
...
def request_conversion():
    ...
    if ext_req in ["mp4", "mkv", "mpeg", "webm_video"]:
        ...
        ydl_opts["postprocessors"] = [
            {"key": "FFmpegVideoConvertor", "preferedformat": ext_req}
        ]
...
def api_jadwal_status():
    user_id = (
        g.user.get("sub") if has_request_context() and hasattr(g, "user") else None
    )
...
def get_photo(role, id_):
    ...
    # Fetch dari Sicyca
    logging.info(
        f"Fetching foto untuk tombol: {role}/{id_}"
    )  # Ubah log ke "tombol" untuk clarity
...
def api_krs_data():
    ...
    try:
        ...
        return jsonify(
            {"success": True, "data_krs": krs_list, "masa_studi": masa_studi_text}
        )
...
def api_krs_detail():
    ...
    if not req_type:
        return jsonify(
            {"success": False, "message": "Parameter 'type' wajib diisi."}
        ), 400
    ...
    if data.get("mk"):
        params["mk"] = data.get("mk")
    if data.get("kls"):
        params["kls"] = data.get("kls")
    if data.get("grup"):
        params["grup"] = data.get("grup")
    if data.get("nik"):
        params["nik"] = data.get("nik")  # Untuk materi kuliah
...
def api_sks_data():
    ...
    try:
        ...
        return jsonify({"success": True, "data": sks_data})
...
def sync_cookies():
    ...
    try:
        ...
        if not user_id:
            return jsonify(
                {"success": False, "message": "User ID tidak ditemukan"}
            ), 401
        ...
        if cookie_jar:
            ...
            logging.info(
                f"[Sync Cookies] Mengirim {len(cookies_list)} cookies untuk User ID {user_id}"
            )
        else:
            logging.info(
                f"[Sync Cookies] Tidak ada session aktif di DB untuk User ID {user_id}"
            )
        return jsonify({"success": True, "cookies": cookies_list})

    except Exception as e:
        logging.error(f"[Sync Cookies] Error: {e}")
        return jsonify(
            {"success": False, "message": f"Server Error: {str(e)}", "cookies": []}
        ), 500
...
def get_my_credentials():
    ...
    try:
        ...
        return jsonify(
            {"success": True, "userid": username, "password": decrypted_password}
        )
    except Exception as e:
        ...
        return jsonify(
            {
                "success": False,
                "message": "Terjadi kesalahan saat mengambil kredensial.",
            }
        ), 500
...
def sync_sskm_data():
    ...
    try:
        ...
        if not room_code or room_code not in SSKM_ROOMS:
            return jsonify(
                {"success": False, "error": "Room code invalid or missing"}
            ), 400
        ...
        if data and "rfidData" in data:
            ...
            logging.info(
                f"SSKM data synced for Room {room_code}: {len(SSKM_ROOMS[room_code])} records"
            )
...
def add_sskm_data():
    ...
    try:
        ...
        if is_duplicate:
            ...
            logging.info(
                f"Duplicate scan detected in Room {room_code}: {inputType} {value}"
            )
...
def parse_google_doc(file_id):
    ...
    if drive_service:
        try:
            ...
            if file_meta.get("mimeType") != "application/vnd.google-apps.document":
                return jsonify(
                    {
                        "error": "File yang dipilih bukan Google Doc. Mohon simpan file sebagai Google Doc terlebih dahulu via Drive."
                    }
                ), 400
        except Exception as e:
    ...
    try:
            ...
            for value in elements:
                if "paragraph" in value:
                    ...
                    text += read_structural_elements(
                        value.get("tableOfContents").get("content")
                    )
            return text
        ...
        waktu_match = re.search(
            r"Waktu Pelaksanaan\s*:\s*(.*)", full_text, re.IGNORECASE
        )
        if waktu_match:
...
def sync_metadata_to_doc(logbook_id):
    ...
    try:
        ...
        if waktu_mulai and waktu_selesai:
            try:
                wm = (
                    waktu_mulai.strftime("%d-%m-%Y")
                    if hasattr(waktu_mulai, "strftime")
                    else datetime.strptime(str(waktu_mulai), "%Y-%m-%d").strftime(
                        "%d-%m-%Y"
                    )
                )
                ws = (
                    waktu_selesai.strftime("%d-%m-%Y")
                    if hasattr(waktu_selesai, "strftime")
                    else datetime.strptime(str(waktu_selesai), "%Y-%m-%d").strftime(
                        "%d-%m-%Y"
                    )
                )
        ...
        def update_cell(svc, doc_id, cell, new_text):
            ...
            reqs.append(
                {"insertText": {"location": {"index": c_start}, "text": str(new_text)}}
            )
        ...
        for label, new_value in field_map:
            ...
            for element in doc_content:
                ...
                for row in table.get("tableRows", []):
                    ...
                    if re.search(
                        rf"^{re.escape(label)}\s*:?\s*$", cell_text, re.IGNORECASE
                    ):
                        ...
                        if value_cell:
                            try:
                                ...
                                logging.warning(
                                    f"[Metadata Sync] Failed to update '{label}': {field_err}"
                                )
                    ...
                    if re.search(r"Waktu\s+Pelaksanaan", cell0_text, re.IGNORECASE):
                        ...
                        if len(cells) >= 5:
                            ...
                            if colon_idx is not None:
                                ...
                                if sampai_idx is not None and sampai_idx + 1 < len(
                                    cells
                                ):
                                    ...
                                    try:
                                        ...
                                        doc_content_tmp = document.get("body").get(
                                            "content"
                                        )
                                        # Re-find the row
                                        ...
                                        logging.warning(
                                            f"[Metadata Sync] Failed to update Waktu Pelaksanaan: {e}"
                                        )
                                else:
                                    ...
                                    try:
                                        update_cell(
                                            service,
                                            file_id,
                                            cells[start_cell_idx],
                                            f"{wm_str} sampai {ws_str}",
                                        )
                                        fields_processed += 1
                                    except Exception as e:
                                        logging.warning(
        ...
        return jsonify(
            {
                "success": True,
                "message": f"Berhasil mengupdate {fields_processed} field metadata ke Google Doc.",
            }
        )
...
def sync_custom_activities(logbook_id):
    ...
    try:
        ...
        cursor.execute(
            "SELECT * FROM logbook_entries WHERE logbook_id = %s ORDER BY tanggal ASC, id ASC",
            (real_logbook_id,),
        )
        ...
        for entry in entries:
            cursor.execute(
                "SELECT path, nama_asli FROM logbook_images WHERE entry_id = %s",
                (entry["id"],),
            )
        ...
        nama_mentor = (
            logbook.get("nama_mentor", "Nama Mentor") if logbook else "Nama Mentor"
        )
        ...
        if current_doc_title and current_doc_title != stored_doc_name:
            try:
                ...
                cursor_name.execute(
                    "UPDATE logbooks SET google_doc_name = %s WHERE id = %s",
                    (current_doc_title, real_logbook_id),
                )
                conn_name.commit()
                cursor_name.close()
                conn_name.close()
                logging.info(
                    f"[Google Doc Sync] Doc name updated: '{stored_doc_name}' -> '{current_doc_title}'"
                )
            except Exception as name_err:
                logging.warning(
                    f"[Google Doc Sync] Failed to update doc name: {name_err}"
                )
        ...
        logging.info(
            f"[Google Doc Sync] Existing sections in doc: {list(existing_sections.keys())}"
        )
        logging.info(
            f"[Google Doc Sync] Months to sync: {list(grouped_entries.keys())}"
        )
        ...
        from controller.LogbookController import html_to_plain_and_formatting
        ...
        for month_key in month_keys_sorted:
            ...
            if month_key in existing_sections:
                ...
                if delete_end > delete_start:
                    logging.info(
                        f"[Google Doc Sync] Deleting existing section '{month_key}': {delete_start}-{delete_end}"
                    )
            ...
            if month_key in existing_sections:
                ...
                insert_index = min(
                    existing_sections[month_key]["startIndex"],
            ...
            service.documents().batchUpdate(
                documentId=file_id, body={"requests": batch1_requests}
            ).execute()
            ...
            for element in doc_content:
                if (
                    "table" in element
                    and element.get("startIndex", 0) >= table_insert_index
                ):
            ...
            if not new_table:
                logging.error(
                    f"[Google Doc Sync] Could not find inserted table for {month_key}"
                )
                continue
            ...
            try:
                service.documents().batchUpdate(
                    documentId=file_id, body={"requests": col_width_requests}
                ).execute()
            except Exception as e:
                ...
                for col_idx, h_text in enumerate(header_texts):
                    if col_idx < len(header_cells):
                        ...
                        text_requests.append(
                            {"insertText": {"location": {"index": idx}, "text": h_text}}
                        )
            ...
            for i, entry in enumerate(month_entries):
                ...
                # Col 2: Deskripsi
                if len(cells) > 2:
                    desc, _ = html_to_plain_and_formatting(entry["deskripsi"])
                    text_requests.append(
                        {
                            "insertText": {
            ...
            text_requests.sort(
                key=lambda req: req["insertText"]["location"]["index"], reverse=True
            )

            if text_requests:
                service.documents().batchUpdate(
                    documentId=file_id, body={"requests": text_requests}
                ).execute()
                ...
                if (
                    ...
                    if styled_table:
                        ...
                        for row_idx, row in enumerate(all_rows):
                            ...
                            # Terapkan Rich Text Formatting ke Deskripsi (Kolom ke-2)
                            if row_idx > 0 and len(row.get("tableCells", [])) > 2 and row_idx - 1 < len(month_entries):
                                entry = month_entries[row_idx - 1]
                                _, formatting = html_to_plain_and_formatting(entry["deskripsi"])
                                desc_cell = row.get("tableCells", [])[2]
                                c_content = desc_cell.get("content", [])
                                if c_content:
                                    c_start = c_content[0].get("startIndex", 0)
                                    # Apply specific styling ranges
                                    for rs, re_idx, st in formatting:
                                        abs_start = c_start + rs
                                        abs_end = c_start + re_idx
                                        # Skip if past cell limits
                                        if abs_end >= c_content[-1].get("endIndex", c_start):
                                            abs_end = c_content[-1].get("endIndex", c_start) - 1
                                        ...
                                        fields = []
                                        if st.get("bold"):
                                            text_style["bold"], fields = True, fields + ["bold"]
                                        if st.get("italic"):
                                            text_style["italic"], fields = True, fields + ["italic"]
                                        if st.get("strikethrough"):
                                            text_style["strikethrough"], fields = True, fields + ["strikethrough"]
                                        if st.get("underline"):
                                            text_style["underline"], fields = True, fields + ["underline"]
                                        if st.get("code"):
                                            text_style["weightedFontFamily"] = {"fontFamily": "Courier New"}
                                            fields.append("weightedFontFamily")
                                        if st.get("link"):
                                            text_style["link"] = {"url": st["link"]}
                                            fields.append("link")
                ...
                for i in range(len(month_entries) - 1, -1, -1):
                    ...
                    for img in images:
                        ...
                        try:
                            ...
                            logging.warning(
                                f"[Google Doc Sync] Image insert failed for entry {entry_id}, url={img_url}: {img_err}"
                            )
            ...
            if resume_content_raw:
                ...
                for element in doc_content:
                    ...
                    if found_header and "table" in element:
                        resume_insert_index = element.get(
                            "endIndex", resume_insert_index
                        )
                        break
                ...
                resume_body, resume_fmt = html_to_plain_and_formatting(resume_content_raw)
                ...
                # Apply rich text formatting from resume_fmt
                body_start = resume_insert_index + len(resume_header) + 1
                for rs, re_idx, st in resume_fmt:
                    abs_start = body_start + rs
                    abs_end = body_start + re_idx
                    if st.get("indent"):
                ...
                service.documents().batchUpdate(
                    documentId=file_id, body={"requests": resume_requests}
                ).execute()
            ...
            try:
                ...
                logging.info(
                    f"[Google Doc Sync] Sig block for {month_key}: found_header={found_header}, found_table={found_table}, sig_insert_index={sig_insert_index}"
                )
                ...
                try:
                    ...
                    nearby_paragraphs = [
                        ...
                        if "paragraph" in el
                        and el.get("startIndex", 0) < sig_insert_index
                    ]
                    if nearby_paragraphs:
                        # Ambil 2 paragraf terakhir sebelum insert point
                        check_para = (
                            nearby_paragraphs[-2]
                            if len(nearby_paragraphs) >= 2
                            else nearby_paragraphs[-1]
                        )
                ...
                service.documents().batchUpdate(
                    documentId=file_id, body={"requests": sig_requests}
                ).execute()
            except Exception as sig_err:
                logging.error(
                    f"[Google Doc Sync] FAILED to insert signature block for {month_key}: {sig_err}"
                )
            ...
            if should_insert_ttd:
                ...
                logging.info(
                    f"[Google Doc Sync] Inserting TTD image for {month_key}: {ttd_url}"
                )
                ...
                try:
                    ...
                    for element in doc_content:
                        if (
                            ...
                            and element.get("startIndex", 0) >= sig_insert_index
                        ):
                            for elem in element.get("paragraph", {}).get(
                                "elements", []
                            ):
                    ...
                    logging.warning(
                        f"[Google Doc Sync] TTD image insert failed for {month_key}: {ttd_err}"
                    )
        ...
        for element in doc_content:
            if "paragraph" in element:
                ...
                if "Aktivitas Bulan" in txt:
                    match = txt.strip().replace("Aktivitas Bulan ", "").strip()
                    if match:
                        cleanup_section_headers.append(
                            (match, element.get("startIndex"))
                        )
        ...
        if orphaned_months:
            logging.info(
                f"[Google Doc Sync] Cleaning up orphaned sections: {orphaned_months}"
            )
            # Proses dari index terbesar ke terkecil supaya tidak geser index
            orphaned_sorted = sorted(
                orphaned_months,
                key=lambda m: cleanup_sections[m]["startIndex"],
                reverse=True,
            )
            ...
            for orphan_month in orphaned_sorted:
                ...
                if delete_end > delete_start:
                    try:
                        ...
                        orphaned_count += 1
                        logging.info(
                            f"[Google Doc Sync] Deleted orphaned section '{orphan_month}'"
                        )
                    except Exception as cleanup_err:
                        logging.warning(
                            f"[Google Doc Sync] Failed to delete orphaned section '{orphan_month}': {cleanup_err}"
                        )
        ...
        if entries:
            msg_parts.append(
                f"Berhasil mengekspor {len(entries)} kegiatan ({len(month_keys)} bulan)"
            )
        if orphaned_count > 0:
            msg_parts.append(
                f"{orphaned_count} bulan lama berhasil dihapus dari Google Doc"
            )
...
def api_prayer_global_hijri_calendar():
    ...
    return jsonify({"success": bool(days), "month": month, "year": year, "days": days})
...
def api_payment_transactions():
    ...
    result, status_code = handle_list_transactions(
